<?php
	...
	$rs = mysql_query("Select * from tratamento;"); // **LINHA IMPORTANTE
?>

<!DOCTYPE html>
<html lang="br">
<head>
  <title>Listar Tratamento</title>
  <meta charset="utf-8">
...
<h1 align="center"><img src="tratamento_banner.png" class="img-rounded" alt="Cinque Terre" width="450" height="200" align="center"></h1>
	<br/>
			...
			<?php 
				while($row=mysql_fetch_array($rs)){ ?> <!-- row - variavel, pode ter outro nome como $linha-->
					...
					<tr>
						...
						<td width="150"><?php echo $row['descricao']?> </td>
						<td width="150"><?php echo $row['preco']?> </td>
						<td >
							<input type="button" class="btn btn-info" value="Editar" 
							onclick="javascript: location.href='editarTratamentos.php?id=' + <?php echo $row['id'] ?>"/>
						</td>
						<td >
							<input type="button" id="btRem" name="btRem" class="btn btn-danger" value="Remover"
							onclick="javascript: location.href='frmRmvTratamentos.php?id=' + <?php echo $row['id'] ?>"/>
						</td>
					</tr>
...
<h1 align="center"><button type="button" class="btn btn-primary" align="center" id="bt_novo" 
	onclick="javascript: location.href='insTratamento.html' ">Cadastrar Novo</button></h1>
	<br/>
